...
void RL::StateController(const RobotState<float>* state, RobotCommand<float>* command)
{
    ...
    if (this->params.Has("commands_limit_lower") && this->params.Has("commands_limit_upper"))
    {
        const auto lower = this->params.Get<std::vector<float>>("commands_limit_lower");
        const auto upper = this->params.Get<std::vector<float>>("commands_limit_upper");
        if (lower.size() >= 3 && upper.size() >= 3)
        {
            this->control.x = std::clamp(this->control.x, lower[0], upper[0]);
            this->control.y = std::clamp(this->control.y, lower[1], upper[1]);
            this->control.yaw = std::clamp(this->control.yaw, lower[2], upper[2]);
        }
    }
...
std::vector<float> RL::ComputeObservation()
{
    ...
    auto ObservationScaleKey = [this](const std::string& key) -> std::string
    {
        const auto observations_history = this->params.Get<std::vector<int>>("observations_history");
        const std::string actor_history_key = "actor_history_" + key;
        if (!observations_history.empty() && this->params.Has(actor_history_key))
        {
            return actor_history_key;
        }
        return key;
    };
    auto ObservationBoolKey = [this](const std::string& key) -> std::string
    {
        const auto observations_history = this->params.Get<std::vector<int>>("observations_history");
        const std::string actor_history_key = "actor_history_" + key;
        if (!observations_history.empty() && this->params.Has(actor_history_key))
    ...
    for (const std::string &observation : this->params.Get<std::vector<std::string>>("observations"))
    {
        ...
        {
            obs_list.push_back(this->obs.lin_vel * this->params.Get<float>(ObservationScaleKey("lin_vel_scale")));
        }
        else if (observation == "ang_vel")
        {
            ...
            {
                obs_list.push_back(this->obs.ang_vel * this->params.Get<float>(ObservationScaleKey("ang_vel_scale")));
            }
            else if (this->ang_vel_axis == "world")
            {
                obs_list.push_back(QuatRotateInverse(this->obs.base_quat, this->obs.ang_vel) * this->params.Get<float>(ObservationScaleKey("ang_vel_scale")));
            }
        }
        ...
        {
            obs_list.push_back(this->obs.commands * this->params.Get<std::vector<float>>(ObservationScaleKey("commands_scale")));
        }
        else if (observation == "dof_pos")
        {
            std::vector<float> dof_pos_rel = this->obs.dof_pos - this->params.Get<std::vector<float>>("default_dof_pos");
            if (this->params.Get<bool>(ObservationBoolKey("zero_wheel_dof_pos"), true))
            {
            ...
            auto dof_pos_scaled = dof_pos_rel * this->params.Get<float>(ObservationScaleKey("dof_pos_scale"));
            if (this->params.Has("obs_mapping"))
            {
                auto mapping = this->params.Get<std::vector<int>>("obs_mapping");
                std::vector<float> reordered(mapping.size());
                for (size_t i = 0; i < mapping.size(); ++i) reordered[i] = dof_pos_scaled[mapping[i]];
                obs_list.push_back(reordered);
            }
            else
            {
                obs_list.push_back(dof_pos_scaled);
            }
        }
        else if (observation == "dof_vel")
        {
            auto dof_vel_scaled = this->obs.dof_vel * this->params.Get<float>(ObservationScaleKey("dof_vel_scale"));
            if (this->params.Has("obs_mapping"))
            {
                auto mapping = this->params.Get<std::vector<int>>("obs_mapping");
                std::vector<float> reordered(mapping.size());
                for (size_t i = 0; i < mapping.size(); ++i) reordered[i] = dof_vel_scaled[mapping[i]];
                obs_list.push_back(reordered);
            }
            else
            {
                obs_list.push_back(dof_vel_scaled);
            }
        }
        ...
        else if (observation == "ball_pos_b")
        {
            obs_list.push_back(this->obs.ball_pos_b);
        }
        // ============= Other Observations =============
        ...
        else if (observation == "takeoff/phase")
        {
            float motion_time = this->episode_length_buf * this->params.Get<float>("dt") * this->params.Get<int>("decimation");
            float cycle_time = this->params.Get<float>("phase_cycle_time");
            float phase = motion_time / cycle_time;
...
void RL::InitObservations()
{
    ...
    this->obs.commands.assign(std::max<size_t>(this->params.Get<std::vector<float>>("commands_scale").size(), 3), 0.0f);
    ...
    this->obs.ball_pos_b = {0.0f, 0.0f, 0.0f};
...
void RL::InitRL(std::string robot_config_path)
{
    ...
    if (static_cast<int>(this->ComputeObservation().size()) != this->params.Get<int>("num_observations"))
    {
        throw std::runtime_error(
            "Observation dimension mismatch for " + robot_config_path +
            ": config num_observations=" + std::to_string(this->params.Get<int>("num_observations")) +
            ", computed=" + std::to_string(this->ComputeObservation().size())
        );
    }
    this->InitOutputs();
    this->InitControl();
