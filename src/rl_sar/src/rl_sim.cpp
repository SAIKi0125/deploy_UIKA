...
RL_Sim::RL_Sim(int argc, char **argv)
{
    ...
    request->names = {"robot_name", "gazebo_model_name", "catchball_debug"};
    ...
    if (status == rclcpp::FutureReturnCode::SUCCESS)
    {
        ...
        else
        {
            ...
            this->catchball_debug = result->values[2].bool_value;
            ...
            std::cout << LOGGER::INFO << "Get param catchball_debug: " << (this->catchball_debug ? "true" : "false") << std::endl;
        }
    }
    else
    {
...
bool IsRobotBaseLinkName(const std::string& name, const std::string& gazebo_model_name)
{
    return (!gazebo_model_name.empty() && name == gazebo_model_name + "::base")
        || name == "robot_model::base"
        || name == "go2w_gazebo::base";
}

bool IsCatchballLinkName(const std::string& name)
{
    return name.rfind("catchball::", 0) == 0;
}

std::vector<float> PositionToVector(const geometry_msgs::msg::Point& position)
{
    return {
...
std::vector<float> QuaternionToWxyz(const geometry_msgs::msg::Quaternion& orientation)
{
    return {
...
{
    std::lock_guard<std::mutex> lock(this->model_states_mutex);

    int ball_idx = -1;
    for (size_t i = 0; i < msg->name.size(); ++i)
    {
        if (msg->name[i] == "catchball")
        {
            ball_idx = static_cast<int>(i);
        }
    }

    this->has_ball_model_pose = ball_idx >= 0;
    if (this->has_ball_model_pose)
    {
        this->ball_model_pose = msg->pose[ball_idx];
    }

    this->UpdateCatchballObservationLocked();
}
...
void RL_Sim::LinkStatesCallback(const gazebo_msgs::msg::LinkStates::SharedPtr msg)
{
    ...
    int robot_base_idx = -1;
    int ball_link_idx = -1;
    for (size_t i = 0; i < msg->name.size(); ++i)
    {
        if (IsRobotBaseLinkName(msg->name[i], this->gazebo_model_name))
        {
            robot_base_idx = static_cast<int>(i);
        }
        else if (IsCatchballLinkName(msg->name[i]))
        {
            ball_link_idx = static_cast<int>(i);
        }
    }

    this->has_robot_base_link_pose = robot_base_idx >= 0;
    if (this->has_robot_base_link_pose)
    {
        this->robot_base_link_pose = msg->pose[robot_base_idx];
    }

    this->has_ball_link_pose = ball_link_idx >= 0;
    if (this->has_ball_link_pose)
    {
        this->ball_link_pose = msg->pose[ball_link_idx];
    }

    this->UpdateCatchballObservationLocked();
}

void RL_Sim::UpdateCatchballObservationLocked()
{
    if (this->manual_ball_pos_override_enabled)
    {
        return;
    }

    if (!this->has_robot_base_link_pose || (!this->has_ball_link_pose && !this->has_ball_model_pose))
    {
        this->obs.ball_pos_b = {0.0f, 0.0f, 0.0f};
        return;
    }

    const auto& ball_pose = this->has_ball_link_pose ? this->ball_link_pose : this->ball_model_pose;
    const bool in_flight = ball_pose.position.z >= 0.05;

    this->obs.ball_pos_b = this->catchball_perception.ComputeBallPosB(
        PositionToVector(this->robot_base_link_pose.position),
        QuaternionToWxyz(this->robot_base_link_pose.orientation),
        PositionToVector(ball_pose.position),
        in_flight);
}
#endif
